...
class EmailServiceTest {
  ...
  class TestEmailError {
    ...
    @Test
    @DisplayName("Throws SupabaseConnectionException when "
        + "WebClient fails during wypozyczenia fetch")
    void testWypozyczeniaFetchThrowsSupabaseConnectionException() {
      when(webClient.get()).thenReturn(requestHeadersUriSpec);
      when(requestHeadersUriSpec.uri(any(Function.class))).thenThrow(
          new RuntimeException("WebClient failure"));
